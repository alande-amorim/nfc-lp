...
exports.onCreateNode = ({ node, actions }) => {
  ...
  if (node.internal.type === "SpecificationsJson") {
    // extends the existing gatsby node with a new field, later accessible via the fields graphql node.
    createNodeField({
      node, // the current node
      name: `specification`, // defines a name for the new element being added.
      value: `../assets/specifications/${node.image.src}`, //Injects the value, this will be relative to the path of the json, it will look into /src/images
    })
  }
